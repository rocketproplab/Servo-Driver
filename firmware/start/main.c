...
static void i2c_callback(const struct i2c_s_async_descriptor *const descr)
{
	// receive i2c command
	uint8_t command;
	...
	switch (command) {
		case CMD_SET_POS: {
			...
			// send confirmation
			io_write(io, on_batt ? &CONF_FAILURE : &CONF_SUCCESS, 1);
			break;
		}
...
int main(void) {
	...
	i2c_s_async_register_callback(&I2C_0, I2C_S_RX_COMPLETE, i2c_callback);
